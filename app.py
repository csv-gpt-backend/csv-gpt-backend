...
app = FastAPI(
    title="CSV + GPT Backend",
    description="Descarga 2 CSV desde Wix y responde preguntas estrictamente con GPT-5.",
    version="1.0.0",
)

# ============ CONFIG ==============
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")  # config en Render → Settings → Environment
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-5")  # forzamos gpt-5 por defecto
MAX_ROWS = 100  # filas máximas por CSV

# URLs fijas de tus CSV en Wix
CSV_URLS = [
...
# CORS (permitir llamadas desde Anvil)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # en prod, cámbialo a ["https://TU-APP.anvil.app"]
...
@app.get("/preview")
def preview():
    """Devuelve confirmación con nombres, columnas y primeras filas"""
    previews = []
    for u in CSV_URLS:
        data = fetch_csv(u)
        previews.append({
